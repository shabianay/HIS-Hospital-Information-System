                ...
                <tr data-search-row x-show="!search || $el.textContent.toLowerCase().includes(search.toLowerCase())" class="hover:bg-secondary-50 dark:hover:bg-secondary-900/30 transition-colors">
                    ...
                    <td class="py-4 px-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <x-action-link href="{{ route('patients.show', $patient) }}">Lihat</x-action-link>
                            <x-action-link href="{{ route('patients.edit', $patient) }}" variant="warning">Edit</x-action-link>
                            <x-action-delete action="{{ route('patients.destroy', $patient) }}" confirm="Hapus pasien ini?">Hapus</x-action-delete>
                        </div>
                    </td>
                </tr>
                @empty
