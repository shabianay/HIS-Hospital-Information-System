...
<div class="w-full space-y-6">
    <h2 class="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">Buat Permintaan Radiologi</h2>

    <form action="{{ route('radiology.requests.store') }}" method="POST" x-data="{ urgent: false, selectedCount: 0 }" @change="selectedCount = document.querySelectorAll('input[name=\'radiology_test_ids[]\']:checked').length">
        @csrf
        ...
        <div class="mt-8 bg-surface-light dark:bg-surface-dark p-8 rounded-2xl border border-border-light dark:border-border-dark shadow-glass-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <label class="flex items-center justify-between gap-3 rounded-xl border border-border-light dark:border-border-dark px-4 py-3 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-colors cursor-pointer">
                        <span class="flex items-center gap-3">
                            <input type="checkbox" name="radiology_test_ids[]" value="{{ $test->id }}"
                                class="h-4 w-4 rounded border-border-light text-primary-600 focus:ring-primary-500 dark:border-border-dark dark:bg-surface-dark dark:ring-offset-background-dark">
            ...
            @empty
            <p class="text-sm text-text-secondary-light dark:text-text-secondary-dark">Belum ada pemeriksaan radiologi yang aktif. Tambahkan di <a href="{{ route('radiology.tests') }}" class="text-primary-600 dark:text-primary-400 font-semibold">Master Pemeriksaan</a>.</p>
            @endforelse

            <div>
                <x-input-label value="Catatan Klinis (Opsional)" />
                <textarea name="clinical_notes" rows="3" placeholder="Contoh: Suspek pneumonia, klinis batuk 2 minggu..." class="w-full border border-border-light bg-surface-light px-4 py-3 text-sm text-text-primary-light focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 focus:outline-none rounded-xl shadow-glass-sm dark:border-border-dark dark:bg-surface-dark dark:text-text-primary-dark"></textarea>
            </div>
            ...
            <div class="mt-6 flex items-center justify-between">
                ...
                <div class="flex gap-3">
                    <x-secondary-button href="{{ route('radiology.requests') }}">Batal</x-secondary-button>
                    @if($appointment)
