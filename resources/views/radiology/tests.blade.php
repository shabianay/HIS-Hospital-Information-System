@extends('layouts.app')
@section('title', 'Master Pemeriksaan Radiologi')
@section('content')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 class="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">Master Pemeriksaan Radiologi</h2>
        <div class="flex gap-2">
            <a href="{{ route('radiology.tests.csv') }}" class="inline-flex items-center justify-center px-5 py-2.5 border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-dark text-sm font-semibold rounded-xl shadow-glass-sm transition-all duration-200 hover:bg-secondary-50 dark:hover:bg-secondary-900/30">Export CSV</a>
    ...
    <div x-data="{ showAdd: false }">
        <dialog x-ref="addForm" @click.self="showAdd = false" :open="showAdd"
            ...
            <div class="bg-surface-light dark:bg-surface-dark rounded-2xl border border-border-light dark:border-border-dark shadow-glass-lg p-8">
                <h3 class="text-lg font-bold text-text-primary-light dark:text-text-primary-dark mb-6">Tambah Pemeriksaan Radiologi</h3>
                <form method="POST" action="{{ route('radiology.tests.store') }}" class="grid grid-cols-1 gap-5">
                    @csrf
                    <div>
                        <x-input-label value="Nama Pemeriksaan" />
                        <x-text-input type="text" name="name" required placeholder="Contoh: Foto Thorax PA" />
                    </div>
                    <div>
                        <x-input-label value="Kategori" />
                        <x-text-input type="text" name="category" placeholder="Contoh: Rontgen / USG / CT Scan / MRI" />
                    </div>
                    <div>
                        <x-input-label value="Satuan" />
                        <x-text-input type="text" name="unit" placeholder="Contoh: proyeksi" />
                    </div>
                    <div>
                        <x-input-label value="Keterangan" />
                        <x-text-input type="text" name="reference_range" placeholder="Contoh: Persiapan puasa 6 jam" />
                    </div>
                    <div>
    ...
    <div class="bg-surface-light dark:bg-surface-dark p-8 rounded-2xl border border-border-light dark:border-border-dark shadow-glass-sm">
        <x-table placeholder="Cari pemeriksaan radiologi...">
            <x-slot name="head">
                <tr class="border-b border-border-light dark:border-border-dark text-xs uppercase tracking-wider text-text-secondary-light dark:text-text-secondary-dark">
                    ...
                    <th class="pb-4 px-4 font-semibold">Keterangan</th>
            ...
            @empty
            <tr x-show="!search" data-search-row><td colspan="7" class="py-6 text-center text-text-secondary-light dark:text-text-secondary-dark">Belum ada pemeriksaan radiologi.</td></tr>
            @endforelse
        </x-table>
        <div class="mt-6">
            {{ $tests->links() }}
