        ...
        <form action="{{ route('medicines.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <x-text-input type="text" name="generic_name" value="{{ old('generic_name') }}" />
                </div>
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        <x-input-label for="category" value="Kategori" />
                        <x-text-input type="text" name="category" value="{{ old('category') }}" placeholder="Misal: Analgesik" />
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <x-text-input type="number" name="buy_price" value="{{ old('buy_price') }}" min="0" required />
                        <x-input-error :messages="$errors->get('buy_price')" />
                    </div>
                    <div>
                        <x-input-label for="sell_price" value="Harga Jual (Rp)" />
                        <x-text-input type="number" name="sell_price" value="{{ old('sell_price') }}" min="0" required />
                        <x-input-error :messages="$errors->get('sell_price')" />
                    </div>
                </div>
                <div>
                    <x-input-label for="minimum_stock" value="Stok Minimum" />
                    <x-text-input type="number" name="minimum_stock" value="{{ old('minimum_stock') }}" min="0" required />
