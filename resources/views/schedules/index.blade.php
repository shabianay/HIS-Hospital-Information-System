                ...
                <tr data-search-row x-show="!search || $el.textContent.toLowerCase().includes(search.toLowerCase())" class="hover:bg-secondary-50 dark:hover:bg-secondary-900/30 transition-colors">
                    ...
                    <td class="py-4 px-4 text-sm text-text-primary-light dark:text-text-primary-dark">
                        <div class="flex items-center gap-2">
                            <x-action-link href="{{ route('schedules.show', $schedule) }}">Lihat</x-action-link>
                            <x-action-link href="{{ route('schedules.edit', $schedule) }}" variant="warning">Edit</x-action-link>
                            <x-action-delete action="{{ route('schedules.destroy', $schedule) }}" confirm="Hapus jadwal ini?">Hapus</x-action-delete>
                        </div>
                    </td>
                </tr>
                @empty
