...
<div class="mx-auto max-w-2xl">
    <div class="bg-surface-light dark:bg-surface-dark p-8 rounded-2xl border border-border-light dark:border-border-dark shadow-glass-sm">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-border-light dark:border-border-dark">
            <h2 class="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">Tambah Jadwal Baru</h2>
        </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <x-text-input type="time" name="start_time" value="{{ old('start_time') }}" required />
                        <x-input-error :messages="$errors->get('start_time')" />
                    </div>
                    <div>
                        <x-input-label for="end_time" value="Jam Selesai" />
                        <x-text-input type="time" name="end_time" value="{{ old('end_time') }}" required />
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <x-text-input type="number" name="daily_quota" value="{{ old('daily_quota') }}" min="1" max="200" required />
                        <x-input-error :messages="$errors->get('daily_quota')" />
                    </div>
                    <div>
                        <x-input-label for="consultation_fee" value="Biaya Konsultasi (Rp)" />
                        <x-text-input type="number" name="consultation_fee" value="{{ old('consultation_fee') }}" min="0" required />
