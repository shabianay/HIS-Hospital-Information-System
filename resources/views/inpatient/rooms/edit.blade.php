...
<div class="w-full">
    <div class="bg-surface-light dark:bg-surface-dark p-8 rounded-2xl border border-border-light dark:border-border-dark shadow-glass-sm">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-border-light dark:border-border-dark">
            <h2 class="text-2xl font-bold text-text-primary-light dark:text-text-primary-dark">Edit Kamar: {{ $room->name }}</h2>
        </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <x-text-input type="text" name="code" value="{{ old('code', $room->code) }}" required />
                        <x-input-error :messages="$errors->get('code')" />
                    </div>
                    <div>
                        <x-input-label for="name" value="Nama Kamar" />
                        <x-text-input type="text" name="name" value="{{ old('name', $room->name) }}" required />
