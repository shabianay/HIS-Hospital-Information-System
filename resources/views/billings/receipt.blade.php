    ...
    <div class="mx-auto max-w-md bg-white p-6 shadow-md border border-gray-200">
        ...
        <div class="space-y-1 text-xs">
            <div class="flex justify-between"><span>Subtotal:</span><span>Rp
                    {{ number_format($billing->total_amount + $billing->discount, 0, ',', '.') }}</span></div>
            @if ($billing->discount > 0)
                <div class="flex justify-between"><span>Diskon:</span><span>- Rp
                        {{ number_format($billing->discount, 0, ',', '.') }}</span></div>
            @endif
            <div class="flex justify-between font-bold"><span>Total Tagihan:</span><span>Rp
                    {{ number_format($billing->total_amount, 0, ',', '.') }}</span></div>
            <div class="flex justify-between"><span>Dibayar:</span><span>Rp
