        ...
        <header class="flex items-center justify-between border-b border-border-dark/50 pb-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-text-secondary-dark">Display Antrian Laboratorium</p>
                </div>
            </div>
        ...
        <main class="my-6 grid flex-1 grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-8 flex flex-col items-center justify-between rounded-2xl border border-border-dark/50 bg-surface-dark p-8 text-center">
                <div>
                    ...
                    <div class="mt-2 text-2xl font-semibold text-secondary-300">Silakan Menuju ke Loket Laboratorium</div>
                    <div class="mt-4 flex flex-wrap items-center justify-center gap-2">
                        ...
                        <span x-show="current && current.is_urgent" class="rounded-lg bg-danger-600 px-3 py-1 text-sm font-bold">URGEN</span>
                    </div>
                </div>
                <div class="mt-6 w-full border-t border-border-dark/50 pt-6">
                    <p class="text-sm text-text-secondary-dark">Mohon persiapkan lembar permintaan laboratorium Anda.</p>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 flex flex-col gap-4">
                <div class="rounded-2xl border border-border-dark/50 bg-surface-dark p-5">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="rounded-lg bg-secondary-700 px-2 py-1 font-mono text-sm font-bold text-primary-400" x-text="waiting.length">0</span>
                    </div>
                    <div class="mt-4 space-y-3 max-h-[70vh] overflow-y-auto">
                        ...
                        <template x-for="(item, index) in waiting" :key="item.id">
                            <div class="rounded-xl border border-border-dark/50 bg-background-dark/50 p-4">
                                <div class="flex items-center justify-between">
                                    <span class="font-mono text-2xl font-black text-primary-400" x-text="String(index + 1).padStart(2, '0')"></span>
                                    <span x-show="item.is_urgent" class="rounded bg-danger-600 px-2 py-0.5 text-xs font-bold">URGEN</span>
                                </div>
                                ...
                                <div class="mt-1 flex flex-wrap gap-1.5">
                                    <template x-for="test in item.tests.slice(0, 3)" :key="test">
                                        <span class="rounded bg-secondary-700 px-2 py-0.5 text-xs" x-text="test"></span>
                                    </template>
                                    <span x-show="item.tests.length > 3" class="rounded bg-secondary-700 px-2 py-0.5 text-xs" x-text="'+' + (item.tests.length - 3)"></span>
                                </div>
                                <p class="mt-1 text-xs text-secondary-500">Permintaan <span x-text="item.created_at"></span></p>
                            </div>
                        </template>
                    </div>
                </div>
